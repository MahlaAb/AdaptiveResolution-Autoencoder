...
#----------------------------------------------
# down-scale an input tensor x by a factor=factor
def adapt_scale(x, factor):
    x = tf.nn.avg_pool(x, ksize=[1, 1, factor, factor], strides=[1,1,factor,factor], padding='VALID', data_format='NCHW')    
    return x
#----------------------------------------------------------------------------
...
def conv2d(x, fmaps, kernel, **kwargs):
    ...
    kernel = tf.get_variable(
        name='w',
        shape=[kernel, kernel, x.get_shape()[1], fmaps],
        dtype=tf.float32,
        initializer=tf.truncated_normal_initializer(stddev=stddev)
    )
...
#----------------------------------------------------------------------------
# Style modulation. from https://github.com/NVlabs/stylegan
...
#----------------------------------------------------------------------------
# Noise input. from styleGAN https://github.com/NVlabs/stylegan
